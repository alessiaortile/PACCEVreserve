...
def plot_error_calibration(
    ...
    ax2.set_ylabel("Mean positive z-score")

    ax2.set_xlabel(r"Privacy level $\epsilon$")
    ...
    ax1.invert_xaxis()
    ax2.invert_xaxis()
    ax2.set_ylabel(r"Mean positive bias / $\sigma$")
...
def make_plots(summary: pd.DataFrame) -> None:
    ...
    colors = {
        ...
        "CDP_NoPP": ["#008B8B","#008B8B"],      # brown + neutral gray
    }
    ...
    plot_error_calibration(
        eps_vals=sorted(summary["eps"].unique(),reverse=True)[1:],ldp_flex_error=ldp_flex_error,cdp_flex_error_pre=cdp_flex_error_pre,
                    cdp_flex_error_post=cdp_flex_error_post,ldp_zscore=ldp_zscore,cdp_zscore_pre=cdp_zscore_pre,cdp_zscore_post=cdp_zscore_post)
...
def main():
    ...
    NoPP90 = list(RESULTS_DIR.glob("SOC_B_bidvalidation_summary_records_eps???_100_CDP_NoPP.csv"))
    ...
    CDP90_extra = RESULTS_DIR / "SOC_B_bidvalidation_summary_records_100_0.9.csv"
    NoPP90_extra = RESULTS_DIR / "SOC_B_bidvalidation_summary_records_100_0.9.csv"
    ...
    for path in sorted(NoPP90, key=parse_eps):
      rows.append(load_summary(path, method="CDP_NoPP"))
    ...
    if NoPP90_extra.exists():
        row = load_summary(NoPP90_extra, method="CDP_NoPP")
        row["eps"] = 12
        rows.append(row)
    else:
        print(f"Warning: Extra deterministic file not found at {NoPP90_extra}")
